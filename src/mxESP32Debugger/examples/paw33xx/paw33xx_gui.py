...
sys.path.append(os.path.join(os.path.dirname(__file__), "../../../"))  # Add the debugger package to the path
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.config = toml.load(os.path.join(os.path.dirname(__file__), "./paw3395.toml"))
        self.image_size = self.config["image"]["width"] * self.config["image"]["height"]
        
        self.reset_pin = 39
        self.dev.gpio_config(pin=self.reset_pin, mode=0x02, pull_up=True, pull_down=False)
        self.dev.power_control("spi", self.config["electronic"]["vdd_voltage"])
    ...
    def update_image(self):
        ...
        self.dev.gpio_write_read(self.reset_pin, 0)
        self.dev.gpio_write_read(self.reset_pin, 1)
        ret, data = self.dev.spi_read_image(self.config['image']['width'], self.config['image']['height'])
        ...
        if len(data) == self.image_size:
            ...
            data = np.array(data)
            data = data.reshape(self.config['image']['width'], self.config['image']['height'])
